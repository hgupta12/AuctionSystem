...
In classical RL, the major mathematical modelling is based on a *Markov Decision Process* (**MDP**). Formally, an MDP is a 4-tuple(S, A<sub>s</sub>, P<sub>a</sub>, R<sub>a</sub>), wherein:
...
Q<sup>*</sup>(s, a) = R(s, a) + $\gamma$ $\sum$<sub>s'$\epsilon$ S</sub> P<sub>ss'</sub>(a) $\max$<sub>a' $\epsilon$ A(s')</sub> Q<sup>*</sup>(s', a') **(2)**
